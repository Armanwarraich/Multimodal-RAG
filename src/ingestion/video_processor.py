...
class VideoProcessor:
    ...
    def process(
        self,
        video_dir: str
    ) -> tuple[list[Document], list[Image.Image], list[str]]:
        ...
        for file in sorted(Path(video_dir).iterdir()):
            ...
            try:
                ...
                print(
                    "[VideoProcessor] No audio track found, "
            ...
            for timestamp, frame_img in frames:
                ...
                keyframe_sources.append(
                    f"{file.name}::frame_{timestamp:.1f}s"
                )

        print(
            f"[VideoProcessor] Transcripts: {len(transcript_docs)}, "
    ...
    def _extract_audio(
        ...
        if not out_path.exists():
            raise FileNotFoundError(
                f"Audio file not created: {out_path}"
            )
    ...
    def apply_temporal_attention(self, vectors):
        print(
            f"[VideoProcessor] Applying temporal attention over "
            f"{vectors.shape[0]} frames..."
        )

        attended = self.temporal_attn.attend(vectors)

        print("[VideoProcessor] Temporal attention applied.")
    ...
    def unload(self):
        ...
        if self._audio_tmp.exists():
            ...
            print("[VideoProcessor] Cleaned up temp audio files.")
